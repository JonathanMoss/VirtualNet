...
def handle_ptt_request(db, station: Station, sid: str, station_registry, broadcast_roster):
    """Process PTT request from a station, enforcing lock availability and CONTROL override."""
    ...
    # Channel busy - check if requesting station is CONTROL (NCS Override)
    if station.role == "CONTROL":
        ...
        register_transmitting_sid(sid, net_id)
        broadcast_roster(db, net_id)
        return {"allowed": True, "transmissionId": new_tx.id}
